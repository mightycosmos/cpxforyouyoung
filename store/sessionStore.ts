...
import { listPastExamsFromFirestore } from '@/lib/firebase/pastExams';
...
interface SessionState {
  ...
  /** 기출 모드 사용자 등록 기출 (Firestore 동기화) */
  userPastExams: PastExam[];
  ...
  saveUserPastExam: (exam: PastExam) => Promise<string>;
  removeUserPastExam: (id: string) => Promise<boolean>;
  reset: () => void;
  updateReviewMemo: (sessionId: string, reviewMemo: string) => Promise<void>;
...
function mergeUserPastExamsByUpdatedAt(...lists: PastExam[][]): PastExam[] {
  const byId = new Map<string, PastExam>();
  for (const list of lists) {
    for (const item of list) {
      if (!item?.id || item.isPreMade) continue;
      const prev = byId.get(item.id);
      if (!prev || item.updatedAt >= prev.updatedAt) {
        byId.set(item.id, item);
      }
...
function readPersistedUserPastExamsFromDisk(uid: string): PastExam[] {
  if (typeof window === 'undefined') return [];
  try {
    const key = `${BASE_PERSIST_KEY}:${uid}`;
    const raw = window.localStorage.getItem(key);
    if (!raw) return [];
    const parsed = JSON.parse(raw) as { state?: { userPastExams?: PastExam[] } };
    const arr = parsed.state?.userPastExams;
    return Array.isArray(arr) ? arr.filter((e) => !e.isPreMade).slice(0, 200) : [];
  } catch {
    return [];
  }
}
  ...
  persist(
    (set, get) => ({
      ...
      loadUserDataFromCloud: async (uid: string) => {
        try {
          ...
          let legacyPastExams: PastExam[] = [];
          try {
            const legacyAll = await listPastExamsFromFirestore();
            legacyPastExams = legacyAll.filter(
              (exam) => !exam.isPreMade && (!exam.authorId || exam.authorId === uid)
            );
          } catch (legacyErr) {
            console.warn('Legacy pastExams migration skipped:', legacyErr);
          }
          ...
          set((state) => {
            ...
            const mergedUserPastExams = mergeUserPastExamsByUpdatedAt(
              settings.userPastExams ?? [],
              legacyPastExams,
              readPersistedUserPastExamsFromDisk(uid),
              state.userPastExams ?? []
            );
            ...
            return {
              ...
              userPastExams: mergedUserPastExams,
      ...
      syncUserSettingsToCloud: async (opts?: { includeDraftMemo?: boolean }) => {
        try {
          ...
          const base = {
            ...
            userPastExams: (s.userPastExams ?? []).filter((exam) => !exam.isPreMade),
      ...
      saveDirectCase: async ({ id: existingId, title, systemCategory, chiefComplaint, caseSpec, formPayload }) => {
        ...
        if (!ok) {
          console.warn('Firestore Cloud sync skipped or failed; local persistence succeeded.');
        }
        return id;
      },
      ...
      saveUserPastExam: async (exam) => {
        const id = exam.id?.trim() || crypto.randomUUID();
        const now = Date.now();
        const entry: PastExam = {
          ...exam,
          id,
          updatedAt: now,
          isPreMade: false,
          authorId: getFirebaseAuth().currentUser?.uid,
        };
        set((state) => ({
          userPastExams: [entry, ...(state.userPastExams ?? []).filter((e) => e.id !== id)].slice(0, 200),
        }));
        const ok = await get().syncUserSettingsToCloud();
        if (!ok) {
          console.warn('Firestore Cloud sync skipped or failed; local persistence succeeded.');
      ...
      removeUserPastExam: async (id) => {
        set((state) => ({
          userPastExams: (state.userPastExams ?? []).filter((e) => e.id !== id),
        }));
        return get().syncUserSettingsToCloud();
      },
      ...
      updateReviewMemo: async (sessionId, reviewMemo) => {
        set((state) => ({
          archivedSessions: state.archivedSessions.map((s) =>
            s.id === sessionId ? { ...s, reviewMemo } : s
          ),
        }));
        ...
        try {
          ...
          console.error('Failed to sync updated review memo to Firestore:', error);
        }
      },
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        userPastExams: state.userPastExams,
