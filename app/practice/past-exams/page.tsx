...
function PastExamsContent() {
  ...
  const { startSession, bookmarkedPastExamIds, togglePastExamBookmark, userPastExams, saveUserPastExam, removeUserPastExam } = useSessionStore();
  ...
  const [sourceFilter, setSourceFilter] = useState<'ALL' | 'PRE_MADE' | 'USER_MADE'>('ALL');
  ...
  const loadExams = useCallback(async () => {
    ...
    try {
      ...
      const userExams = (useSessionStore.getState().userPastExams ?? []).filter((exam) => !exam.isPreMade);
  ...
  useEffect(() => {
    ...
    void loadExams();
  }, [authLoading, user, router, loadExams, userPastExams]);
  ...
  const filteredExams = useMemo(() => {
    return exams.filter((exam) => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchesName = exam.caseSpec.patient.name.toLowerCase().includes(query);
        const matchesDx = exam.caseSpec.true_diagnosis.toLowerCase().includes(query);
        if (!matchesTitle && !matchesName && !matchesDx) return false;
      }
      // Source Filter (사전 기출 / 내가 제작한 기출)
      if (sourceFilter === 'PRE_MADE' && !exam.isPreMade) {
        return false;
      }
      if (sourceFilter === 'USER_MADE' && exam.isPreMade) {
        return false;
      }
      return true;
    });
  }, [exams, categoryFilter, presentationFilter, showBookmarksOnly, bookmarkedPastExamIds, searchQuery, sourceFilter]);
  ...
  const buildFallbackCaseSpec = (payload: DirectCaseFormPayload, targetId: string): CaseSpec => {
    const dxRanked = payload.diagnosisRanked || ['추정진단 1순위', '감별진단 A', '감별진단 B'];
    const trueDx = dxRanked[0] || payload.chiefComplaint;
    const differentials = [dxRanked[1] || '감별진단 A', dxRanked[2] || '감별진단 B'];

    const bp = payload.vitals?.bp || '120/80';
    const hr = payload.vitals?.hr ? Number(payload.vitals.hr) || 80 : 80;
    const rr = payload.vitals?.rr ? Number(payload.vitals.rr) || 20 : 20;
    const temp = payload.vitals?.temp ? Number(payload.vitals.temp) || 36.5 : 36.5;

    const historyBlocks = payload.historyBlocks || {};
    const hpiLines = Object.entries(historyBlocks)
    ...
    return {
      ...
      chief_complaint_display: payload.chiefComplaintText || payload.chiefComplaint,
      clinical_presentation: payload.chiefComplaint,
      difficulty: payload.difficulty || 'normal',
      true_diagnosis: trueDx,
      differentials: differentials,
      opening_line: payload.chiefComplaintText || '어디가 불편해서 오셨나요?',
      ...
      patient: {
        name: payload.patientName || '환자',
        age: payload.patientAge || 30,
        gender: payload.patientGender || '남',
        occupation: '무직',
        education: '대졸',
      },
      ...
      history: {
        hpi,
        past_medical: historyBlocks['과'] || historyBlocks['과거력'] || '없음',
        medications: historyBlocks['약'] || historyBlocks['약물력'] || '없음',
        allergies: historyBlocks['알레르기'] || '없음',
        family: historyBlocks['가'] || historyBlocks['가족력'] || '없음',
        social: {
          smoking: historyBlocks['사']?.includes('담배') ? historyBlocks['사'] : '비흡연',
          alcohol: '음주 안 함',
          occupation: '무직',
        },
      },
      symptom_details: {
        onset: historyBlocks['O'] || '최근',
        location: historyBlocks['L'] || '해당 부위',
        duration: historyBlocks['D'] || '지속적',
        character: historyBlocks['C'] || '통증/불편감',
        progression: historyBlocks['Co'] || '보통',
        aggravating: historyBlocks['F'] || '없음',
        relieving: '없음',
        associated: historyBlocks['A'] || '없음',
        denied: '없음',
      },
      personality: payload.specialOther || '보통 태도',
      patient_concern: payload.specialQuestion || '걱정 없음',
      physical_exam_findings: payload.physicalExtraLines?.join('\n') || '정상 소견',
      checklist: [
        { category: '기본 문진', items: ['주증상 및 발병시기 확인', '과거력 및 약물력 확인', '동반증상 확인'] },
        { category: '신체진찰 및 진단계획', items: ['기본 신체진찰 시행', '추정진단 및 감별진단 설명', '검사 및 치료계획 설명'] }
      ],
      answer_key: {
        diagnosis_ranked: [trueDx, differentials[0] || '감별진단 A', differentials[1] || '감별진단 B'],
        management_plan: {
          tests: payload.managementPlanTests || '필요한 검사 시행 및 결과 확인',
          treatment: payload.managementPlanTreatment || '적절한 치료 및 처방',
        },
        patient_education: payload.patientEducation || '질환의 경과 및 주의사항 교육',
      },
    };
  };
  ...
  const handleRegisterExam = async () => {
    ...
    try {
      ...
      if (managementPlanTests.trim()) {
        payload.managementPlanTests = managementPlanTests.trim();
      }
      if (managementPlanTreatment.trim()) {
        payload.managementPlanTreatment = managementPlanTreatment.trim();
      }
      if (patientEducation.trim()) {
      ...
      try {
        ...
        if (compRes.ok) {
          ...
          caseSpec = compData.caseSpec;
        } else {
          caseSpec = buildFallbackCaseSpec(payload, targetId);
        }
      } catch {
        caseSpec = buildFallbackCaseSpec(payload, targetId);
      }
      ...
      // Save to user settings (Firestore users/{uid}/settings/app)
      await saveUserPastExam(examData);
          ...
          <aside className="col-span-12 md:col-span-4 lg:col-span-3 space-y-6">
            <div className="glass rounded-3xl border border-black p-5 relative overflow-hidden">
              ...
              <div className="relative z-10 space-y-5">
                ...
                {/* Source Filter Tabs */}
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-black/50 uppercase tracking-wider block">기출 분류</label>
                  <div className="grid grid-cols-3 gap-1 border border-black/10 rounded-xl p-1 bg-white/40">
                    {[
                      { id: 'ALL' as const, label: '전체' },
                      { id: 'PRE_MADE' as const, label: '사전 기출' },
                      { id: 'USER_MADE' as const, label: '내 기출' },
                    ].map((tab) => (
                      <button
                        key={tab.id}
                        type="button"
                        onClick={() => setSourceFilter(tab.id)}
                        className={`text-center py-1.5 rounded-lg text-[10px] font-bold transition-all cursor-pointer ${
                          sourceFilter === tab.id
                            ? 'bg-black text-white'
                ...
                {/* Text Search Input */}
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-black/50 uppercase tracking-wider block">검색어 (환자명/진단명)</label>
                  <div className="relative">
                    <input
                      ...
                      placeholder="검색어 입력..."
                      className="w-full rounded-2xl border border-black px-3 py-2 text-sm bg-white/50 outline-none pr-8"
                    />
                    {searchQuery && (
                      <button
                        type="button"
                        onClick={() => setSearchQuery('')}
                        className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs font-bold text-black/40 hover:text-black cursor-pointer"
                      >
                        ✕
                      </button>
                    )}
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-1.5 mb-2">
                            ...
                            {exam.isPreMade ? (
                              ...
                              <span className="text-[9px] font-extrabold text-emerald-600 bg-emerald-50 border border-emerald-200 rounded px-1">
                                내가 제작한 기출
                              </span>
                            )}
                          </div>
