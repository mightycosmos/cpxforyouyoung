...
export default function ArchiveDetailPage() {
  ...
  const { archivedSessions, updateReviewMemo } = useSessionStore();
  ...
  useEffect(() => {
    if (session) {
      setReviewMemo(session.reviewMemo ?? '');
    }
  }, [session]);
  ...
  const handleSaveReviewMemo = async () => {
    ...
    setSavingMemo(true);
    try {
      await updateReviewMemo(params.id, reviewMemo);
      alert('리포트 메모가 저장되었습니다.');
    } catch (error) {
      console.error(error);
      alert('메모 저장에 실패했습니다.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto px-5 py-6 space-y-6">
        ...
        {/* 리포트 메모 (오답노트) */}
        {scoreResult && (
          <div className="border border-neutral-100 rounded-2xl p-5 bg-neutral-50/50 space-y-3">
            ...
            <textarea
              ...
              placeholder="이 증례에 대한 피드백이나 오답 노트를 적어보세요..."
              className="w-full min-h-[100px] rounded-xl border border-neutral-200 bg-white p-3 text-sm text-black outline-none focus:border-black/50 transition-all resize-y leading-relaxed"
            />
            <div className="flex justify-end">
              <button
                ...
                disabled={savingMemo}
                className="px-4 py-2 rounded-xl bg-black text-white text-xs font-bold hover:bg-black/90 disabled:opacity-50 active:scale-[0.98] transition-all cursor-pointer flex items-center gap-1.5"
              >
                {savingMemo ? (
                  <>
