...
export async function loadUserSettings(userId: string): Promise<UserSettingsDoc> {
  ...
  return {
    ...
    userPastExams: Array.isArray(d.userPastExams) ? d.userPastExams.slice(0, 200) : [],
...
export async function saveUserSettings(
  ...
  partial: Pick<
    UserSettingsDoc,
    'examTimeDeductionSeconds' | 'memoTemplates' | 'directCases' | 'userPastExams' | 'draftMemoContent' | 'bookmarkedPastExamIds'
  >
): Promise<void> {
