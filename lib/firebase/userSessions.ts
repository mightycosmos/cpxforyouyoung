...
function docToSessionData(id: string, data: Record<string, unknown>): SessionData | null {
  try {
    ...
    return {
      ...
      isPastExam: data.isPastExam !== undefined ? Boolean(data.isPastExam) : undefined,
      pastExamTitle: data.pastExamTitle !== undefined ? String(data.pastExamTitle) : undefined,
      reviewMemo: data.reviewMemo !== undefined ? String(data.reviewMemo) : undefined,
    };
  } catch {
    return null;
  }
